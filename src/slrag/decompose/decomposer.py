...
class Decomposer:
    ...
    def __init__(self, config_dir: str | Path = "config") -> None:
        ...
        # Loaded from the pipeline baked into models/ at build time (HC-1/G1: never downloaded
        # at runtime). The previous in-function `import spacy.cli` shadowed `spacy` and raised
        # UnboundLocalError before any model could load.
        self.nlp = None
        if HAS_SPACY:
            try:
                from slrag.synth.lexicon import spacy_model
