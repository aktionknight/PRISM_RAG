...
def test_session_state_destroy():
    ...
    session.claims.append(Claim(claim_id="c1", facet="test", text="claim text", citations=[]))
...
async def test_orchestrator_process_stream():
    ...
    orchestrator.process_chunk = AsyncMock(return_value=ControllerDecision(
        t_s=0.0, decision=ControllerDecisionType.WAIT, reason=ControllerReason.intent_unstable, confidence=1.0
    ))
